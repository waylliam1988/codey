...
- 自动模式下，主模型的首次正常调用直接作答，或请求一个受权限检查的动作
  （chat、只读 planning、Research、Writer 或 Review；Hybrid 通过显式模式选择）。
  没有独立的路由模型调用；项目写锁只在真正选定编辑动作后才获取。
...
已验证结果引用），并在下一次正常模型调用前按严格 token 预算检索少量相关过往回合。
