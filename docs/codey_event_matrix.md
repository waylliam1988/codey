...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| run_event.turn | codey.runtime.observe.events.RunEvent.turn_started | task_run, run_ledger, sse:chat_stream | agent_runner | run_ledger | false | sse:chat_stream | false | false | UI/SSE and ledger projection only; Review model-visible recent log is declared separately |
| run_event.tool_start | codey.runtime.observe.events.RunEvent.tool_started | task_run, run_ledger, sse:chat_stream | tool_runtime | run_ledger | false | sse:chat_stream | true | false | tool name, display path, activity, and optional command projection only |
| run_event.tool | codey.runtime.observe.events.RunEvent.tool_finished | task_run, run_ledger, sse:chat_stream | tool_runtime | run_ledger | false | sse:chat_stream | true | false | presentation result, status, booleans, exit code, and managed-output metadata only |
| run_event.info | codey.runtime.observe.events.RunEvent.info | task_run, run_ledger, sse:chat_stream | agent_runner | run_ledger | false | sse:chat_stream | false | false | bounded status text and optional provider names |
| run_event.status | codey.runtime.observe.events.RunEvent.status | task_run, run_ledger | agent_runner | run_ledger | false | false | false | false | bounded local progress text only |
| sse.run_event_ui_payload | codey.runtime.observe.events.run_event_ui_payload | task_run, sse:chat_stream | agent_runner | none | false | sse:chat_stream | false | false | existing UI/SSE payload projection only; no extra durable copy |
...
| runtime_operation.state | codey.runtime.core.operation_state | task_run, run_details | runtime_operations | runtime_session_log | false | false | false | false | leaf, driver, turn/tool coordinates, provider id, turn counts, proof refs/status, repair round count, repair-context digest, and bounded blocked reason only; pending effect ids and delivery batch refs are derived from `codey.runtime.log.session_view`, not stored; prompts, replies, stdout, diffs, and source bodies are excluded |
| run_trace.prompt_sections | codey.runtime.observe.prompt_envelope | run_trace, prompt_envelope | prompt_envelope | run_trace | false | false | false | true | section names, source refs, lengths, truncation flags, and hashes only |
...
| review.recent_log | codey.runtime.observe.events.render_run_event | review_runner, prompt_envelope, run_trace | review_runner | none | true | false | true | true | bounded recent run log rendered for Review prompt, including writer reply and tool summaries from the current run |
...
| tool_outcome.presentation | codey.toolchain.runtime | codey.runtime.observe.events, task_run, sse:chat_stream, run_ledger | tool_runtime | none | false | sse:chat_stream | true | false | UI-facing status/result fields with managed-output handle metadata only |
...
| ghost.observations | codey.ghost.observations | agent_runner, prompt_envelope, run_trace, local_context_drawer | local_context | ghost_observations | true | local_context_drawer | false | true | bounded completed-round facts (user words, final answer); retrievable only when committed; idempotent by run_id |
