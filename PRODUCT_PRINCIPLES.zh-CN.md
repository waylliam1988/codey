# Codey 产品原则

这份文档写给未来的自己和后来的维护者。

它不描述某个具体功能怎么写，而是说明 Codey 应该是什么、不应该变成什么，以及后续做功能时该怎么判断取舍。

## 一句话定位

Codey 是一个本地优先、低成本、多网页模型兼容的 AI 编程与研究工作台。

它用用户已经能访问的网页版 AI 作为“大脑”，用本地程序提供网页 AI 缺少的能力：读写项目文件、运行测试、显示 diff、恢复改动、审批 shell、双模型 review、受控 Research、来源笔记和项目 handoff。

Codey 的目标不是复制 Cursor、Claude Code、OpenClaw、Hermes 或 Pi Agent。它应该保持更小、更克制：

> 网页 AI 负责思考，本地 Codey 负责边界、工具、来源、diff、恢复和验证。

## 为什么做 Codey

AI 编程不应该只属于买得起昂贵 API、订阅或大额度套餐的人。

很多人已经能使用网页 AI，但网页 AI 本身不能安全地读写本地项目、跑测试、展示 diff、撤销改动。Codey 要做的就是把这些能力接起来，让普通人可以用更低成本体验真实的 AI 编程闭环。

同样地，网页 AI 可以讨论资料，但它自己说不清哪些网页真实打开过、哪些结论已经落盘、哪些证据可以带进项目。Codey 的 Research 要补上这层边界：搜索、打开网页、写来源笔记、校验证据、生成有上限的 Research Brief，再把结论交给项目 Writer。

Codey 的价值不是“免费替代专业工具”，而是：

- 低门槛：先能用，再慢慢理解 Git、测试、diff。
- 低成本：优先使用用户自己的网页 AI 登录状态。
- 本地优先：代码和项目留在用户电脑上。
- 可见可控：每次改了什么都能看到。
- 可恢复：出错时尽量能 restore 或用 Git 回滚。
- 可验证：鼓励跑测试，而不是只相信模型文字。
- 可追溯：研究结论必须能回到实际打开过的来源。

## 目前 Codey 已经是什么

Codey 已经有一个可用的核心闭环：

1. 用户选择项目。
2. 用户用自然语言描述任务。
3. 网页 AI 通过 JSON 工具协议请求读文件、写文件、跑命令。
4. Codey 在本地执行受控工具。
5. Codey 显示红绿 diff。
6. 没有 Git 时也能 snapshot diff 和 restore。
7. 有 Git 时自动使用 Git diff。
8. shell 命令需要用户批准。
9. 任务失败时可以 retry。
10. 打开两个模型网页时，可以自动让第二个模型 review 代码改动。
11. 用户显式打开 Research 后，可以搜索、打开网页、写来源笔记和 synthesis。
12. Research 结束后，可以把 Research Brief 带进项目 Writer。
13. 项目成功实现并验证后，可以保存 implementation / verification note。
14. 可以配置 OpenAI-compatible 的本地模型作为普通 provider。

这已经让 Codey 具备了“正经软件”的基本形态。

## 该做什么

### 继续加强核心闭环

优先级最高的永远是：

- 模型能稳定连接。
- 文件读写安全。
- diff 清楚。
- restore 可靠。
- shell 审批明确。
- 测试结果能反馈给模型。
- 网页 AI 改版后容易修对应 adapter。

任何新功能如果削弱这些基础能力，都应该暂停。

### 工具结果必须按受众分层

本地工具不能再把一段 `output` 字符串同时给模型、UI、receipt 和审计使用。

工具结果应该明确拆成：

- `model_text`：只给模型看的有界文本。
- `presentation`：只给 UI、SSE 和 receipt 用的展示事实。
- `audit`：只给 RunLedger、RunTrace 或本地审计用的小 metadata。
- `canonical`：程序内部可用的 JSON-safe 结构化事实。

模型 prompt 只能渲染 `model_text`。`presentation`、普通 `audit` 和 `canonical`
不能偷偷进入 prompt。长命令输出只能通过 managed output 本地保存，并用有界 footer
告诉模型“本地保留了审计副本”，不能变成新的读取工具或全文上下文注入。

这些投影必须在结果类型边界变成 JSON-safe 且有界。不能把 `object()`、Path、非有限
float、过深对象、超长字符串或无限列表原样塞进后续 ledger、trace、receipt 或 export。

### 保持 UI 安静

Codey 的 UI 应该像一个安静的本地开发工具，而不是一个功能展示页。

用户不需要看到这些词：

- provider
- skill
- workflow
- agent graph
- planner
- memory
- tool registry

这些可以存在于底层，但不应该压到小白用户脸上。

用户应该只看到：

- 项目
- 聊天
- Research
- 模型选择
- 运行状态
- Local context
- diff
- restore
- retry

用户侧统一叫 `Research`。`Knowledge`、`vault`、`artifact`、`index` 都是内部实现词，不应该出现在主界面入口里。

`Local context` 只作为用户主动打开的本地状态审计入口出现，放在安静的更多菜单里；
它不是常驻侧栏，不是人格面板，也不在普通任务路径里弹出。

### Research 必须是受控闭环

Research 是 Codey 的原生能力，但它不是后台自动记忆，也不是普通聊天的隐藏联网。

必须守住这些边界：

1. 只有用户明确点击 `Research`，才允许本轮使用 `web_search`、`open_url` 和写 Research notes。
2. 普通聊天默认不联网、不写长期笔记。
3. Project Writer 默认没有网页搜索工具；需要研究时先进入 Research，再通过 Research Brief handoff 到项目。
4. Research 报告只能引用本轮实际打开过的 requested URL、final URL 或已经 grounded 的 source note。
5. 模型不能决定 note 属于哪个 session 或 project；归属由本地运行时强制写入。
6. 不把整个 vault 注入 Writer，只注入有上限的 Research Brief。
7. 代码本身留在项目目录；vault 记录为什么这么做、依据是什么、改了哪些文件、跑过什么检查。
8. Restore 必须对应后端真实存在的 snapshot；重启后没有 snapshot 的旧 run 不能假装可恢复。

Research 的生命周期编排必须只有一个 owner。外围运行时可以负责
provider、session、取消、trace 和 UI/SSE 转发，但 proof、plan、bounded
follow-up、最终选择和 Evidence Ledger 归档都应由 ResearchPipeline 统一决定。
单轮运行的工具状态只能通过明确的迭代边界传递，不能偷偷挂在最终结果对象上，
也不能为了迁就旧测试保留无意义的兼容入口。

Research 的价值不是让 Codey 变成另一个资料库 app，而是补齐这条工作流：

```text
Chat -> Research -> Research Brief -> Project -> Implementation memory
```

### 网页自恢复必须有边界

网页模型小幅改版时，Codey 可以在后台自动恢复，但不能把“自动发现”变成任意点击：

1. 永远先使用每个网站经过验证的固定 Profile。
2. 自动发现只覆盖输入框、发送按钮和回答区域。
3. 会触发操作的候选必须唯一，并同时满足语义、位置和可用状态要求。
4. 兄弟模型只能看到有上限、经过脱敏的结构候选，不能看到聊天正文、源码或任意 DOM。
5. 只有确认文字已经填入、消息确实开始发送、回答确实读到之后，才能原子记住整组新控件。
6. 流程恢复只能组合固定布尔事实，禁止 selector、JavaScript、URL、任意点击、网页正文和项目数据。
7. 完成态必须观察到“生成中 → 明确终止”的真实转换；只有文字稳定时宁可不恢复，也不能提前截断回答。
8. 新恢复包先 provisional，下一次自然成功后才 active；明确的连续结构失败恢复上一版。
9. 自动恢复没有把握时，请人点击一次，下一次再自动复用。
10. 登录、验证码和网站下线仍然交给人处理，不尝试绕过。

这套能力应该完全隐藏在底层，不增加开关、状态条或技术提示。

### Provider 故障接管必须保留单 Writer

一个网页模型失败后，Codey 可以换健康兄弟模型继续，但不能把任务变成并发多 Writer：

1. 只对明确类型化的 Provider 网页故障接管，普通代码异常和工具失败不接管。
2. 新 Writer 必须使用干净会话，只接收原任务和有边界的本地 checkpoint 事实。
3. 同一 Provider 在同一任务只尝试一次，最多切换两次，总轮次不能因切换重置。
4. 提交状态不确定时绝不向原 Provider 重发；用户 Stop 永远优先。
5. 登录、验证码和限流只改变健康状态，不触发 Profile Doctor 或自动绕过。

### 用自然语言触发能力

用户不应该先选一个复杂模式再工作。

理想体验是：

```text
修测试
解释这个项目
做一个小游戏
把这个页面改好看一点
```

Codey 后台判断怎么执行。判断不准时，不要硬套流程，继续走普通 agent。

原则是：

> 宁可少识别，也不要乱识别。

### 少而强的工具

Codey 不应该堆一堆半成品工具。

当前最重要的工具很少：

- read
- write/edit
- run shell with approval
- diff
- restore
- review

很多看起来像新能力的东西，其实只是这些工具的组合，不需要做成新按钮。

### 双模型保持克制

双模型最有价值的地方是代码改动后的 review。

推荐模式：

```text
模型 A 写代码
模型 B 看 diff
模型 B 只指出具体问题
模型 A 根据 review 修一次
```

不建议做复杂群聊 UI。普通聊天阶段默认单模型即可。聊天里的第二模型补充意见很容易变成噪音，除非将来有很清楚的产品证据证明它有用。

## 不该做什么

### 不要做提示词 Skill

只靠提示词包装出来的 skill 不够可靠，也不优雅。

例如：

```text
你是 Python 专家，请按照以下步骤修复 bug...
```

这类内容可以作为 prompt 的一部分，但不应该被包装成产品能力。

真正有价值的能力应该是程序控制的流程或工具：

- Codey 负责跑测试。
- Codey 负责收集失败输出。
- Codey 负责限制上下文。
- Codey 负责落盘、diff、restore。
- 模型只负责需要智能判断的部分。

### 写后诊断必须是提示，不是隐式执行

Codey 可以在成功写入后提供低成本、确定性的诊断，但不能借此扩大本地执行边界：

1. 只比较修改前后能够明确证明的回归，不评价原本已经损坏的文件。
2. 诊断不能阻止写入、自动回滚、自动运行命令或冒充绿色检查。
3. 输出只包含有边界的导航事实，不重新泄露源码行，也不猜测修复方案。
4. 第一版只使用标准库检查小型 Python 文件，不引入 Ruff、LSP 或 JS/TS 解析器。
5. 诊断自身失败必须静默降级，永远不能把一次本来成功的 edit 变成失败。

### 不要盲目添加 Actions

`Fix Tests`、`Explain Project`、`Create Small App` 这些能力 Codey 现在已经能通过通用 agent 完成。

未来如果要把它们固定成 action，不是为了新增能力，而是为了让高频任务更稳定、更短、更可测试。

在不知道真实高频使用场景之前，不要先做一套 action/skill 框架。

更好的路线是：

1. 继续使用通用 agent。
2. 观察哪些任务经常出现。
3. 观察哪些任务经常失败。
4. 只把真实高频、真实痛点固化成程序流程。

### 不要做插件市场

短期不做插件市场，不做大量第三方 skill。

原因：

- 安全边界会复杂。
- 小白用户难以理解。
- 维护成本会快速上升。
- 容易从“产品”变成“平台工程”。

Codey 现在更应该做少量内置、可测试、可维护的能力。

### 不要追求全能 Agent

OpenClaw、Hermes 这类全能 agent 的方向可以借鉴，但不要照搬。

Codey 不应该一开始就做：

- 长期后台自主乱跑
- 自动操作账号、邮箱、支付
- 大量插件
- 复杂记忆系统
- 多 agent 图形编排
- 自动执行 shell
- 不受限制地访问全电脑

这些会让系统很快变重，也会增加安全风险。

Codey 应该先把“本地项目编程闭环”做到稳定。

## 关于 Actions 的判断

未来也许可以有 actions，但它们必须满足这些条件：

1. 来自真实高频需求，而不是想象。
2. 能减少失败率或减少重复步骤。
3. 有明确安全边界。
4. 有测试。
5. 不增加主 UI 复杂度。
6. 不替代通用 agent，只是优化高频路径。

如果要做，优先考虑这三类：

### Fix Tests

固定流程：

1. 识别测试命令。
2. 请求用户批准运行测试。
3. 收集失败输出。
4. 让模型修复。
5. 再跑测试。
6. 修到通过或达到上限。

但在真实数据证明它高频之前，不要急着做。

### Explain Project

固定流程：

1. 只读项目。
2. 扫描目录结构。
3. 忽略缓存、大文件、构建目录。
4. 读取 README、配置、入口文件。
5. 给新手解释项目做什么、怎么运行、从哪里看起。

这个 action 风险较低，因为它默认不写文件。

### Create Small App

固定流程：

1. 判断项目是否为空，或用户是否明确要求创建。
2. 创建最小可运行结构。
3. 让模型补业务代码。
4. 做最低限度验证。
5. 展示 diff。

目标不是生成复杂工程，而是先生成一个能运行的小东西。

## 关于任务历史

Codey 现在已经有轻量历史雏形：

- 左侧有项目和聊天列表。
- 聊天消息保存在 localStorage。
- 每次任务会记录用户输入、模型回复、工具调用、shell 审批、错误、review、changes 摘要。
- 可以查看 diff。
- 可以 restore。

未来如果增强，不应该做复杂任务管理器。

更合理的是把一次运行整理成一行轻量“任务收据”：

```text
DONE · 2 files changed · checks passed · restore available        View diff
```

它只整理已有事实，不新增任务管理器；具体红绿 diff 仍然放在右侧 changes drawer。

## 关于数据和统计

### 本地连续性不是训练数据

Codey 可以在后台保存六类有严格边界的小型状态：

- 项目事实：只记录受控工具真实运行成功的命令，每个项目一份，数量有上限。
- 对话摘要：只记录继续任务所需的有限事实，每个近期聊天一份，覆盖更新。
- 工作 checkpoint：只记录接管或恢复所需的改动文件 hash、绿色检查和最后动作。
- 恢复基线：只服务非 Git 项目的 diff 和 restore，Restore 完成后删除。
- Provider 控件与 Flow：只保存经过端到端验证的结构指纹、单阶段布尔规则和一代可回滚恢复包。
- Provider 健康：只保存状态、故障类别、计数、时间戳和熔断截止时间。
- Run Trace / Prompt Envelope：只记录模型可见 section 的 digest、长度、用途和来源 ref，
  不保存 raw prompt、完整聊天、源码正文、网页正文或 provider raw error。

这些状态应当共用原子 JSON 写入，但不能混成一个文件，因为生命周期不同。它们不在主界面显示，不保存完整聊天、Cookie、网页 DOM、URL、源码或人类点击教学记录。Project Facts、对话摘要和健康状态写入失败不能打断正在运行的任务；恢复基线首次落盘失败则必须阻止对应文件写入。

Internal Capability Registry 是代码里的只读边界目录，不是新的持久状态、插件系统、
运行时调度器或权限引擎。它只能把内置能力和 policy / model-visible / durable-state
边界写成可测试事实，不能覆盖用户选择、Router 决策或 PermissionProfile。

Action Policy Pipeline 是本地危险动作的单调 guard，不是插件系统、权限矩阵 UI
或 provider/router 决策器。它只能把文件动作、run、shell、Research URL、
provider fallback 和 managed-output artifact 的 `allow` / `ask_user` / `deny`
边界收成可测试、可审计事实；危险动作 sink 必须显式接收 permission profile，
不能在底层偷偷默认 writer 权限；`deny` 不能被后续 guard 放宽，trace 只能保存有界
decision metadata，不能保存 raw command、URL、stdout/stderr、源码正文或 prompt；
mapping fallback 也必须只接受 digest/ref 形状。
未知 action kind 必须默认 deny；managed-output artifact 虽然是本地审计状态，也必须
通过 writer verification profile 与 size/count guard，不能成为绕过 profile 的写入通道。

Event / Capability Matrix 是文档和测试里的事件账本，不是事件总线、运行时配置、
调度器或 UI 功能入口。它只能登记事件的 producer、consumers、capability、
durable_state、model-visible、UI-visible、policy、trace 和 privacy boundary，
并用测试防止新增隐形通道。Web/SSE 事件投影可以抽到共享函数复用，但不能借机改变
payload shape；RunLedger 和机器可读 run-event payload 服务不同消费者，不应为了
“统一”强行合并。由运行事件渲染给 Review prompt 的 recent log 是单独的模型可见
投影，必须在矩阵里显式声明 Prompt Envelope / Run Trace 边界，不能被 UI/SSE 行掩盖。

Built-in Profiles 是后台默认策略目录，不是用户 UI、配置平台、插件系统、Router、
权限引擎或 prompt patch 机制。它只能声明内置倾向和边界，例如 Research-heavy、
Review-strict、Local-only、Beginner；不能覆盖用户显式 provider / mode，不能放宽
PermissionProfile，不能自动联网 Research，Local-only 不能声明 Research permission
default，不能覆盖用户显式 Local context updates 开关。v1 只做 metadata 和测试，
让 Codey 后台更有秩序，不把新概念推给用户。

未来的训练 Episode 是另一类长期数据，必须单独设计、明确启用、脱敏和筛选，不能直接把本地连续性文件当训练数据库。

如果未来想知道哪些 action 值得做，可以先做本地统计，但必须克制。

可以记录任务形状：

```json
{
  "kind": "project_task",
  "provider": "deepseek",
  "result": "done",
  "changed_files": 2,
  "shell_used": true,
  "test_like_command": true,
  "stop_reason": "done"
}
```

不应该记录完整代码、完整 prompt、隐私文件内容。

这些统计只为判断产品方向，不应该变成监控用户。

## 设计取舍原则

每次想加功能时，先问：

1. 这个功能是不是用户已经反复遇到的真实问题？
2. 它能不能通过现有 agent 自然完成？
3. 如果能完成，为什么还要固定成流程？
4. 它会不会让 UI 变复杂？
5. 它会不会扩大安全边界？
6. 它有没有测试？
7. 它失败时用户能不能恢复？

如果答案不清楚，先不做。

## 最重要的一句话

Codey 应该是一个小而稳的本地 AI 编程与研究工作台，不是一个为了展示架构而膨胀的 agent 平台。

它的长期生命力来自：

- 低成本
- 本地优先
- 多网页模型兼容
- diff 可见
- restore 可恢复
- 测试可验证
- UI 克制
- 失败后能修

保持这个方向，比堆更多概念更重要。
