...
- Effect Intent / Settlement Sandwich and Replay Policy v1:
  ...
  - Pre-Gated Recovery & Full Lifecycle Clean Exit: unconfirmed effect recovery is pre-gated before any work-queue claiming, Ghost auto router decisions, or provider sends; recovery failures and operation start failures cleanly exit via a unified `_fail_early()` handler with standard `task_done` error events, cleared `RunRegistry` busy states, and halted external execution; `complete_or_block_work_item()` consumes `GhostWorkItem | None` directly without permissive wrapper fallbacks.
