...
- Safe Tool Replay & Resumption Recovery v1:
  ...
  - Operation Gate Upgrade: replaced the settlement-only resume path with `_recover_effects_for_resume()` in `task_run.py`, validating project paths, project/hybrid writer context, and policy approval before replaying safe tool calls. No legacy recovery wrapper is retained. Wired `recovered_tool_outcomes` through `RunFrame` and consumed/cleared in `_run_one_writer_attempt()`. Runs with recovered tool outcomes skip work-queue claim and auto routing so replayed results return to the same writer path. Unsafe, provider, repair, invalid, or non-writer effects fail-closed to synthetic `interrupted` settlements.
  - Testing & Verification: added unit tests in `tests/test_safe_tool_replay.py`, smoke harness `tests/manual/safe_tool_replay_smoke.py` (`--self-test`), updated `test_tool_replay_policy.py`, `test_runtime_effect_records.py`, `test_agent_effect_sandwich.py`, and `test_runtime_session_log.py`. CLI run-event rendering now uses ASCII tool markers for readable Windows smoke logs. Full pytest passed with `3383 passed, 16 skipped`.
