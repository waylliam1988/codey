...
- Effect Intent / Settlement Sandwich and Replay Policy v1:
  ...
  - Strict Schema & Fail-Closed validation: `from_payload()` enforces strict type/length constraints and rejects missing semantic fields without permissive fallbacks; `record_settlement()` validates category consistency and rejects conflicting duplicate settlements while idempotently returning identical duplicates; `load_effects()` verifies entry/payload lane and operation_id boundary alignment.
  - Fail-Closed Resume Recovery with Full Lifecycle: `_settle_pending_effects_for_resume()` and `_start_run_operation()` fail closed on corrupted logs or synthesis failures, dispatching standard `task_done` error events, clearing `RunRegistry` busy states, releasing claimed work items, and halting external execution immediately.
  ...
  - Tool execution ordering: adjusted order to `execute_tool_call -> record_tool_outcome -> record_tool_call_settlement`; `record_settlement_safely()` guarantees logging errors never mask real outcomes/exceptions; removed obsolete `begin_tool_call()`.
