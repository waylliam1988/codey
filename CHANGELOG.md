...
- Effect Intent / Settlement Sandwich and Replay Policy v1:
  ...
  - Strict intent validation before settlement: `record_settlement()` verifies intent existence, preventing orphan settlements; added `record_settlement_safely()` to ensure logging failures never mask real outcomes or exceptions.
  - Adjusted tool execution order to `execute_tool_call -> record_tool_outcome -> record_tool_call_settlement`, ensuring model-visible tool results are preserved first; removed dead `begin_tool_call()`.
  ...
  - Crash recovery projection: on session resume, unconfirmed pending effects are projected and settled with synthetic `interrupted` records; `recovery_summary` only reports settled interrupted effects and ignores in-flight pending effects to prevent false UI warnings.
