...
- Effect Intent / Settlement Sandwich and Replay Policy v1:
  ...
  - Strict Schema & Fail-Closed validation: `from_payload()` enforces strict type/length constraints, requires `session_id`, `lane`, `operation_id`, `turn`, and `tool_index`, enforces string type on enum fields (`effect_category`, `replay_class`, `status`, `sent_state`) before membership tests via `_require_enum_str` to eliminate raw TypeErrors, and rejects missing semantic fields without permissive fallbacks; `record_settlement()` validates category consistency and rejects conflicting duplicate settlements while idempotently returning identical duplicates; `load_effects()` verifies entry/payload `session_id`, lane, and operation_id boundary alignment and parses in strict chronological order (rejects duplicate intents, orphan settlements, and conflicting settlements).
  - Fail-Closed Operation Start & Resume Recovery: `_start_run_operation()` validates `RuntimeOperationStore.start()` returns a non-None operation and `_settle_pending_effects_for_resume()` succeeds, failing closed on failures with standard `task_done` error events, clearing `RunRegistry` busy states, single-transitioning work items via `run_ghost_post_turn`, and halting external execution immediately; `complete_or_block_work_item()` consumes `GhostWorkItem | None` directly without permissive wrapper fallbacks.
  ...
  - Tool execution ordering: adjusted order to `execute_tool_call -> record_tool_outcome -> record_tool_call_settlement`; `record_settlement_safely()` guarantees logging errors never mask real outcomes/exceptions; removed obsolete `begin_tool_call()` and unused `ReplayDecision.to_payload()`.
