...
- Tool Prompt Consolidation & Runtime Prompt Surface v1:
  ...
  - Consolidated Research contracts: `codey.research.tool_contract.ToolContract` now carries `example` and `description` for the static `Tools:` block (`render_research_tool_contract_text()`); dynamic examples in `tool_example()` converge on authentic cold-start capabilities, explicitly reflecting exact note title support (`dst:"<note id or exact title>"`) in `knowledge_link`; added test assertions locking this choice in both `tool_example("knowledge_link")` and controller allowed-action blocks. Added `research_tool_names()` and `research_tool_contract_hash()` hashing visible research tool text through `model_visible_contract_hash()`.
  ...
  - Added thin `codey.runtime.prompt_surface`: `PROMPT_SURFACE_SCHEMA_VERSION`, `PromptSurfaceSection`, `PromptSurfaceRecord`, `prompt_surface_id()` (now `phase+send_ref+prompt_digest` per-send, not content-dedup); added shared canonicalizers `canonical_surface_phase()`, `canonical_surface_send_ref()`, `canonical_surface_prompt_digest()`; `build_prompt_surface_record()` normalizes fields before generating `surface_id`; `validate_prompt_surface_payload()` strictly rejects non-canonical `phase` (length > 40, whitespace, or invalid characters), strictly bounds `send_ref` to identifier characters (`^[A-Za-z0-9._:-]{1,80}$`, rejecting whitespace/newlines/CRLF), strictly checks unpadded `prompt_digest` without silent stripping, requires non-empty `epoch_id` matching `ctx_epoch:[0-9a-f]{16}`, strictly recomputes and verifies that `surface_id` is an authentic derived value, eliminates bool-int loopholes on `schema_version`, `prompt_chars`, and section `chars` via `type(x) is int`, and removed the no-op `provider_effect_id` check block and dead `_is_epoch()` helper. `PromptSurfaceRecord` is per provider-send attempt (`surface_id` = send identity, `prompt_digest` = content identity, `send_ref` = `provider_effect_id` for writer / `research_send:{n}` for research); same bytes sent twice produce two distinct surfaces.
  - Extended `codey.runtime.prompt_envelope.record_provider_send_prompt()`: now accepts `phase`, `send_ref`, `provider_effect_id`, `model_tool_contract_hash` and `runtime_tool_contract_hash`, prioritizing atomic batch delivery via `trace.record_provider_prompt_boundary()` (single flush) before falling back to individual FailOpenPromptTrace projections. Removed implicit fallback from `send_ref` to `provider_effect_id`; without an explicit `phase`/`send_ref` no surface is recorded, avoiding `writer` pollution for `chat`/`review`/`handoff` calls. The call remains fail-open; `TaskCancelled`/`DeadlineExceeded` still propagate.
  - Extended `codey.runs.trace`: added `MAX_PROMPT_SURFACES`, `PromptSurfaceTrace` (now with `send_ref` and `schema_version`) and `prompt_surfaces` on `RunTraceManifest` (`to_payload()` emits `prompt_surfaces`); extracted private helpers `_append_prompt_section()` and `_append_prompt_surface()`; added `record_provider_prompt_boundary()` for single-flush atomic prompt section and surface persistence before provider sends; added `RunTraceRecorder.record_prompt_surface()` which validates via `validate_prompt_surface_payload()`, requires `send_ref` and `schema_version==1`, deduplicates only by `surface_id` (per-send), and stores only bounded metadata (no raw prompt/reply/stdout/stderr/diff/source bodies). Existing `prompt_sections` stream is unchanged.
  ...
  - Testing & Verification: added `tests/test_tool_prompt.py`, `tests/test_tool_contract_drift.py` (locking `knowledge_link` exact title capability and validating parser repairs against declared kinds), `tests/test_prompt_surface.py` (covering per-send `surface_id` via `send_ref`, authentic derived `surface_id` validation, rejection of non-canonical phase, rejection of newline/space/unstripped send_ref, rejection of padded prompt_digest and missing epoch, atomic provider boundary flush, bool-int rejection, empty surfaces when `send_ref` is omitted, and stored payload field identity assertion) and `tests/test_golden_parity.py` (asserting byte-identical `SYSTEM_PROMPT`/control-block prompts, and authentic `test_research_repair_prompt_golden` recomputing `render_research_repair_prompt` + `append_block` to prevent repair drift with domain-isolated tool contract import). Full pytest suite passed with `3468 passed, 4 skipped, 1235 subtests passed in 300.86s (0:05:00)`. No model-visible coding bytes changed, and research contracts accurately reflect real capabilities without dead fallbacks.
