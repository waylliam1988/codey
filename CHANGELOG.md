...
- Effect Intent / Settlement Sandwich and Replay Policy v1:
  ...
  - Strict Schema & Fail-Closed validation: `from_payload()` enforces strict type/length constraints, requires `lane` and `operation_id`, and rejects missing semantic fields without permissive fallbacks; `record_settlement()` validates category consistency and rejects conflicting duplicate settlements while idempotently returning identical duplicates; `load_effects()` verifies entry/payload lane and operation_id boundary alignment and parses in strict chronological order (rejects duplicate intents, orphan settlements, and conflicting settlements).
  - Fail-Closed Resume Recovery with Full Lifecycle: `_settle_pending_effects_for_resume()` and `_start_run_operation()` fail closed on corrupted logs or synthesis failures, dispatching standard `task_done` error events, clearing `RunRegistry` busy states, releasing claimed work items, invoking post-turn hooks with correct signature, and halting external execution immediately.
