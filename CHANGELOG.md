...
- Tool Prompt Consolidation & Runtime Prompt Surface v1:
  ...
  - Added thin `codey.runtime.prompt_surface`: `PROMPT_SURFACE_SCHEMA_VERSION`, `PromptSurfaceSection`, `PromptSurfaceRecord`, `prompt_surface_id()` (now `phase+send_ref+prompt_digest` per-send, not content-dedup), `build_prompt_surface_record()` (normalizes fields before generating `surface_id`), and `validate_prompt_surface_payload()` with strict `sha256:[0-9a-f]{64}`, `ctx_epoch:[0-9a-f]{16}`, `prompt_surface:[0-9a-f]{16}`, recomputing and verifying that `surface_id` is an authentic derived value, and using `type(x) is int` to eliminate bool-int loopholes on `schema_version`, `prompt_chars`, and section `chars`. Removed dead function `_is_epoch()`. `PromptSurfaceRecord` is per provider-send attempt (`surface_id` = send identity, `prompt_digest` = content identity, `send_ref` = `provider_effect_id` for writer / `research_send:{n}` for research); same bytes sent twice now produce two distinct surfaces.
  - Extended `codey.runtime.prompt_envelope.record_provider_send_prompt()`: now accepts `phase`, `send_ref`, `provider_effect_id`, `model_tool_contract_hash` and `runtime_tool_contract_hash`, still records the existing `record_prompt_section` row and only when an explicit `phase` and `send_ref` are supplied, additionally projects a bounded `record_prompt_surface` summary row via `FailOpenPromptTrace`. Removed implicit fallback from `send_ref` to `provider_effect_id`; without an explicit `phase`/`send_ref` no surface is recorded, avoiding `writer` pollution for `chat`/`review`/`handoff` calls. The call remains fail-open; `TaskCancelled`/`DeadlineExceeded` still propagate.
