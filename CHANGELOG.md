...
- Effect Intent / Settlement Sandwich and Replay Policy v1:
  ...
  - Strict Schema & Fail-Closed validation: `from_payload()` enforces strict type/length constraints without permissive truncations; `record_settlement()` validates category consistency and rejects conflicting duplicate settlements while idempotently returning identical duplicates.
  - Fail-Closed Resume Recovery: `_settle_pending_effects_for_resume()` and `_start_run_operation()` fail closed on corrupted logs or synthesis failures, halting external execution immediately.
