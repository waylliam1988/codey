...
- Safe Replay Result Delivery Receipts & Exact Turn Recovery v1:
  ...
  - Strict Fail-Closed Boundaries: added `can_recover_before_provider_send` requiring zero send attempts; mixed batches containing mutating actions (`edit`, `run`, `shell`) or provider sends strictly fail closed; blocked effect IDs prevent unreplayable batch safe tools from falling back to partial single-effect replay.
  - Agent Loop Refactoring & Prompt Parity: extracted `codey.agents.result_delivery` (`deliver_turn_results`, `deliver_recovered_results`, `build_next_tool_prompt`), eliminating redundant prompt formatting and dispatch code across 3 separate sites in `codey/agents/loop.py`. Guarantees byte-level prompt parity with clean paths and provides fast-path digest reuse via `TurnState`.
  ...
  - Testing & Verification: added unit tests in `tests/test_tool_result_delivery.py` (27/27 passed), manual crash-position smoke in `tests/manual/safe_tool_replay_delivery_smoke.py` (`--self-test` & `--same-run-self-test` passed), and full architecture compliance in `tests/test_architecture.py` (72/72 passed). Full pytest suite passed with `3427 passed, 4 skipped in 296.53s (0:04:56)`.
