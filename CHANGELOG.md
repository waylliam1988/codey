...
- Safe Replay Result Delivery Receipts & Exact Turn Recovery v1:
  ...
  - Strict Fail-Closed Boundaries: added `can_recover_before_provider_send` requiring zero send attempts; mixed batches containing mutating actions (`edit`, `write`, `run`, `shell`) or provider sends strictly fail closed; blocked effect IDs prevent unreplayable batch safe tools from falling back to partial single-effect replay.
  ...
  - Testing & Verification: added unit tests in `tests/test_tool_result_delivery.py` (14/14 passed), manual crash-position smoke in `tests/manual/safe_tool_replay_delivery_smoke.py` (`--self-test` & `--same-run-self-test` passed), and full architecture compliance in `tests/test_architecture.py` (72/72 passed). Full pytest suite passed with `3414 passed, 4 skipped in 300.49s (0:05:00)`.
