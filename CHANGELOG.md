...
- Tool Prompt Consolidation & Runtime Prompt Surface v1:
  ...
  - Normalized `codey.tool_args_repair`: extracted canonical alias tuples `PATH_ARG_KEYS`, `SEARCH_QUERY_KEYS`, `REFERENCES_SYMBOL_KEYS`, `COMMAND_KEYS`, `EDIT_OLD_KEYS`, `EDIT_NEW_KEYS` and the shared `ARG_REPAIR_POLICY` table; normalizers now consume those constants and drift tests read the same constants, eliminating a test-only dead table.
  - Consolidated Research contracts: `codey.research.tool_contract.ToolContract` now carries `example` and `description` for the static `Tools:` block (`render_research_tool_contract_text()`); `tool_example()` intentionally keeps the legacy 0.5.5 dynamic examples (`open_url` with `pages:""`, `knowledge_write` short, `done` with `"<the full report>"/["..."]`) so `controller` allowed-actions and repair prompts remain byte-identical. Added `research_tool_names()` and `research_tool_contract_hash()` hashing the visible research tool text through `model_visible_contract_hash()`. `TOOL_CONTRACTS` static examples now match the exact model-visible `Tools:` block; `research_tool_contract_hash()` no longer hashes a separate required/optional/example structure.
  - Kept `codey.research.protocols` byte parity: retained the hard-boundary and discipline copy, but the `Tools:` list is now injected from `render_research_tool_contract_text()`. `JsonToolCodec.model_tool_contract_hash()` continues to delegate to `research_tool_contract_hash()` with the new text-based hash. Verified that the default research system prompt for both `include_source_search=True/False` remains byte-identical to the pre-0.5.6 rendering (evaluated string, not raw file whitespace).
  ...
  - Added thin `codey.runtime.prompt_surface`: `PROMPT_SURFACE_SCHEMA_VERSION`, `PromptSurfaceSection`, `PromptSurfaceRecord`, `prompt_surface_id()` (now `phase+send_ref+prompt_digest` per-send, not content-dedup), `build_prompt_surface_record()` and `validate_prompt_surface_payload()` with strict `sha256:[0-9a-f]{64}`, `ctx_epoch:[0-9a-f]{16}`, `prompt_surface:[0-9a-f]{16}` and `schema_version==1` enforcement. `PromptSurfaceRecord` is per provider-send attempt (`surface_id` = send identity, `prompt_digest` = content identity, `send_ref` = `provider_effect_id` for writer / `research_send:{n}` for research); same bytes sent twice now produce two distinct surfaces. Removed the unused `PROMPT_TRUST_CLASSES`/`trust_class` wiring that would have become dead code; `validate` now rejects exact forbidden keys with strict hex checks and the comment is corrected to “exact forbidden keys are rejected”.
  - Extended `codey.runtime.prompt_envelope.record_provider_send_prompt()`: now accepts `phase`, `send_ref`, `provider_effect_id`, `model_tool_contract_hash` and `runtime_tool_contract_hash`, still records the existing `record_prompt_section` row and only when an explicit `phase` and `send_ref` (or `provider_effect_id` as `send_ref`) are supplied, additionally projects a bounded `record_prompt_surface` summary row via `FailOpenPromptTrace`. Without an explicit `phase`/`send_ref` no surface is recorded, avoiding `writer` pollution for `chat`/`review`/`handoff` calls. The call remains fail-open; `TaskCancelled`/`DeadlineExceeded` still propagate.
  - Extended `codey.runs.trace`: added `MAX_PROMPT_SURFACES`, `PromptSurfaceTrace` (now with `send_ref` and `schema_version`) and `prompt_surfaces` on `RunTraceManifest` (`to_payload()` emits `prompt_surfaces`); added `RunTraceRecorder.record_prompt_surface()` which validates via `validate_prompt_surface_payload()`, requires `send_ref` and `schema_version==1`, deduplicates only by `surface_id` (per-send), and stores only bounded metadata (no raw prompt/reply/stdout/stderr/diff/source bodies). Existing `prompt_sections` stream is unchanged.
  ...
  - Wired Research runner surface: `codey.research.runner` now maintains a monotonic `_research_send_seq` and on each `_send_provider()` calls `record_provider_send_prompt()` with `phase="research"`, `send_ref="research_send:{n}"`, `model_tool_contract_hash` derived from `controller_action_contract_hash()` when a controller is active (otherwise `codec.model_tool_contract_hash()`), and `runtime_tool_contract_hash` from the codec when a controller is active. Research surfaces are thus per-send even without a runtime effect.
  - Testing & Verification: added `tests/test_tool_prompt.py`, `tests/test_tool_contract_drift.py`, `tests/test_prompt_surface.py` (now covering per-send `surface_id` via `send_ref`, `schema_version` and strict hex validation, `prompt_digest` content identity vs `surface_id` send identity, and that same `prompt_digest` with different `send_ref` yields two surfaces) and `tests/test_golden_parity.py` with committed golden fixtures `tests/fixtures/golden/coding_*`, `research_*`, `controller_*` and `controller_control_block_turn1.txt` asserting byte-identical `SYSTEM_PROMPT`/control-block/repair prompts. Preserved dynamic `tool_example()` legacy examples so `render_control_block()` and repair prompts remain byte-identical to 0.5.5 (verified by `tests/test_research.py` still expecting the short `done` shape). Updated existing imports in `tests/test_tool_definition.py`, `tests/test_protocols.py`, `tests/test_architecture.py` and `tests/test_run_trace.py` to the new `codey.tool_prompt` location. Full pytest suite passed with `3466 passed, 4 skipped, 1235 subtests passed in 291.64s (0:04:51)` after refinement (including the new golden tests). No model-visible prompt bytes were changed, so no live provider A/B is required for this version; the invariant `Tool contract data -> rendered model-visible text -> codec prompt -> outbound prompt digest` is now provable at each step, and the default bytes sent to providers remain identical. A bounded live smoke is optional for release confidence but not a blocker.
