...
Version `0.1.29` adds a hidden Verification Map for Review. It is explicitly a bounded candidate map, not affected-test analysis, impact radius, or coverage proof. After Writer finishes and the final diff is known, Codey scans likely test files once and gives Reviewer explainable evidence: tests changed in the diff, strong Python/JS/TS naming matches, direct Python AST or relative JS/TS imports of changed modules, a small set of declarations added by the diff and referenced by tests, successful checks observed after the latest edit, and broader manifest or previously successful project commands. The scan is bounded to 400 test files, 16MB total attempted test content, 512KB per file, 12 candidates, 20 changed files, 8 observed checks, 6 broader commands, and a 5,000-character rendered result; excluded directories, symlinks, binary/non-UTF-8, secret-like, and oversized files are skipped, attempted non-UTF-8 reads still consume the byte budget, and truncation is explicit. Reviewer is told not to request a test merely because it is a candidate, that every listed candidate is an existing readable local file rather than a missing file, and that it may still use only a changed file as `findings[].path`. TaskRunner and the durable checkpoint both clear successful-check facts after every real edit and after any later failed verification run, so Verification Map, recovery receipts, and successful ProjectFacts cannot inherit stale green checks. No tool, UI, persistence, model call, automatic command execution, dependency graph, or transitive impact claim was added. The release passed 575 pytest tests and 31 subtests, changed-file Ruff, syntax compilation, and `git diff --check`. Real Edge/CDP flows used DeepSeek as Writer and GLM, Qwen, and MiMo as Reviewers; all three approved the independently verified change without a noisy test request after candidate-existence wording was made explicit.

Version `0.1.28` adds a hidden Durable Execution Checkpoint for unfinished project tasks. After a real edit, Codey atomically records only bounded local facts under `~/.codey`: relative changed paths with content hashes, successful check commands that happened after the latest recorded edit, the last edit/run action, and the interruption reason. It does not store source, diffs, tool output, prompts, model plans, or remaining-task claims. A successful edit invalidates earlier checks even if its path cannot be recorded; runtime-accepted paths are canonicalized under the project root before hashing, with the same length boundary on write and reload. On recovery, current file hashes are reconciled and any stale checks are discarded. The checkpoint is injected only when the same session/project is explicitly continued or its provider context is lost for the same task. It survives stop, error, max-turn, and no-progress outcomes, participates in the normal final Diff/Review flow after recovery, and is deleted only after normal completion and any required verified ProjectFacts write. No UI, workflow engine, task decomposition, early Review, or extra model call was added. The release passed 555 pytest tests and 33 subtests, syntax compilation, Ruff on changed code, and `git diff --check`. A real Edge/CDP smoke intentionally stopped DeepSeek after one edit, reopened a fresh model conversation, resumed from the checkpoint, independently passed `unittest`, received GLM approval, and removed the completed checkpoint.
...
Version `0.1.26` briefly introduced `outline_file`, then withdrew it after the natural-use audit. Models consistently preferred the familiar `grep` and offset `read_file` path, while the extra protocol and parser surface carried ongoing maintenance cost. The capability is intentionally removed; Project Map, literal grep, and lexical references remain the supported bounded navigation path.
