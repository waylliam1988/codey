...
`0.1.17` 增加隐藏的 MoA 层，但不增加任何前端控件。只要已经打开了其他支持的模型网页，`New Chat` 可以让它们作为只读顾问提供意见，再由当前选中的模型综合成一条最终回答。空项目或只有占位文件的项目可以先得到一份隐藏顾问方案，适合从零开始集思广益。已有项目会先让顾问做有边界的只读审查：顾问可以列目录、搜索、读取自己认为相关的非敏感文件，但不能改文件、不能运行命令、不能请求 Shell 审批，也不能读取项目外内容。dotfile、env 文件、像密钥的路径、被排除的依赖 / 构建目录、key / 证书文件、锁文件、二进制文件、符号链接和过大的文件都不会发给隐藏项目审查顾问。它们的私有审查报告会交给当前选中的 Writer；Writer 必须基于真实文件验证，仍然是唯一能改文件和跑测试的模型。如果任务完成后没有改文件，最终回答仍可由隐藏共识层补充。顾问失败或没有正常完成会安静降级；如果当前选中的模型已经开始隐藏综合但提交失败，Codey 不会重新发送原始问题，因为第一次网页提交可能已经成功。Review 仍然完全分离：只有真实改过文件后才看最终 Diff。本版的确定性 UI E2E 已覆盖普通聊天共识、项目审查共识、空项目规划，以及原有写入、测试、Review、Restore 全链路，前端没有新模式。
