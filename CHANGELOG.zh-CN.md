...
- 工具提示面收敛与运行时 Prompt 面薄追踪 v1：
  ...
  - 规范化 `codey.tool_args_repair`：抽取标准别名常量 `PATH_ARG_KEYS`、`SEARCH_QUERY_KEYS`、`REFERENCES_SYMBOL_KEYS`、`COMMAND_KEYS`、`EDIT_OLD_KEYS`、`EDIT_NEW_KEYS` 与共享 `ARG_REPAIR_POLICY` 表；规范化器与漂移测试同表共用，消除 test-only 死表。
  - 收敛 Research 契约：`ToolContract` 增加 `example` / `description` 供静态 `Tools:` 块（`render_research_tool_contract_text()`），`tool_example()` 刻意保留 0.5.5 旧动态示例（`open_url` 的 `pages:""`、`knowledge_write` 短示例、`done` 的 `"<the full report>"/["..."]`），使 `controller` 的 allowed-actions 与 repair prompt 保持字节一致；新增 `research_tool_names()` 与基于 `model_visible_contract_hash()` 的 `research_tool_contract_hash()`；`TOOL_CONTRACTS` 静态 example 与模型可见 `Tools:` 块完全一致。
  - 保持 `codey.research.protocols` 字节一致：保留 hard-boundary 与 discipline 文案，`Tools:` 列表改由 `render_research_tool_contract_text()` 注入；`include_source_search=True/False` 两种默认 Research 提示面在求值后与 0.5.6 前完全一致。
  ...
  - 新增薄 `codey.runtime.prompt_surface`：`PROMPT_SURFACE_SCHEMA_VERSION`、`PromptSurfaceSection`、`PromptSurfaceRecord`、`prompt_surface_id()`（现为 `phase+send_ref+prompt_digest` 的 per-send 标识，非内容去重）、`build_prompt_surface_record()` 与 `validate_prompt_surface_payload()`（严格校验 `sha256:`/`ctx_epoch:`/`prompt_surface:` 16 进制与 `schema_version==1`）；`PromptSurfaceRecord` 为每次 provider 发送一条（`surface_id` 为发送身份、`prompt_digest` 为内容身份、`send_ref` 为 `provider_effect_id` 或 `research_send:{n}`），相同内容重发产生不同 surface；已移除未接线的 `PROMPT_TRUST_CLASSES`/`trust_class`，`validate` 的注释改为“exact forbidden keys are rejected”。
  - 扩展 `codey.runtime.prompt_envelope.record_provider_send_prompt()`：在保持原有 `record_prompt_section` 行为外，新增 `phase` / `send_ref` / `provider_effect_id` / `model_tool_contract_hash` / `runtime_tool_contract_hash` 透传，仅当显式 `phase` 与 `send_ref`（writer 为 `provider_effect_id`，research 为 `research_send:{n}`）存在时才额外以 `record_prompt_surface` 投影有界摘要行，无 `send_ref` 则仅保留 `prompt_sections`，避免把 `chat`/`review`/`handoff` 误标为 `writer`；调用保持 fail-open，`TaskCancelled`/`DeadlineExceeded` 仍透传。
  - 扩展 `codey.runs.trace`：新增 `MAX_PROMPT_SURFACES`、`PromptSurfaceTrace`（现含 `send_ref` 与 `schema_version`）与 `RunTraceManifest.prompt_surfaces`，`to_payload()` 输出 `prompt_surfaces`；新增 `RunTraceRecorder.record_prompt_surface()`，经 `validate_prompt_surface_payload()` 严格校验、要求 `send_ref` 与 `schema_version==1`、仅按 `surface_id`（per-send）去重，仅保存有界元数据，不保存原始 prompt/reply/网页正文等。
  - 重排 provider 发送追踪顺序：`codey.agents.prompt_context._send_provider_with_effect()` 先创建 provider effect 并获得 `effect_id`，再以 `phase="writer"`、`send_ref=effect_id` 调用 `record_provider_send_prompt()`，最后 `provider.send()`；`codey.research.runner` 新增单调 `_research_send_seq`，每次 `_send_provider()` 以 `phase="research"`、`send_ref="research_send:{n}"` 调用，`model_tool_contract_hash` 来自 controller 或 codec，`runtime_tool_contract_hash` 在 controller 存在时取 codec；无 `send_ref` 时不记录 surface，保持冷启动干净。
  - 测试与回归：新增 `tests/test_tool_prompt.py`、`tests/test_tool_contract_drift.py`、`tests/test_prompt_surface.py`（现覆盖 per-send `surface_id` 经 `send_ref`、`schema_version` 与严格 hex 校验、`prompt_digest` 内容身份 vs `surface_id` 发送身份、相同 `prompt_digest` 不同 `send_ref` 产生两条 surface、以及 `phase`/`send_ref` 缺失时不记录 surface）与 `tests/test_golden_parity.py`（提交 `tests/fixtures/golden/coding_*`、`research_*`、`controller_*` 与 `controller_control_block_turn1.txt` 黄金基线，锁定 `SYSTEM_PROMPT`/控制块/repair prompt 的字节一致性，保留动态 `tool_example()` 为 0.5.5 旧值以保 repair 字节一致）；迁移旧测试中的 `render_tool_contract`/`model_tool_contract_hash` 引用至 `codey.tool_prompt`。全量 pytest 通过：`3466 passed, 4 skipped`（含新增黄金用例，`291.64s`）。本版未改变任何模型可见字节（`tool_example` 仍为 0.5.5 动态值，静态 `Tools:` 块经 `render` 达成一致），无需线上 provider A/B；不变式 `Tool contract data -> rendered model-visible text -> codec prompt -> outbound prompt digest` 的每一步现已可证明且默认发给 provider 的字节保持不变。
