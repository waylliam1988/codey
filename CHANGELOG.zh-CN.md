...
- 外部副作用 Intent / Settlement 闭环与 Replay Policy v1：
  ...
  - 严格 Schema 与 Fail-Closed：`from_payload()` 严格校验字段类型与长度，要求 `session_id`、`lane`、`operation_id`、`turn` 与 `tool_index` 为必填字段，通过 `_require_enum_str` 严格校验枚举字段（`effect_category`、`replay_class`、`status`、`sent_state`）必须为有效字符串类型，拒绝未知 record_kind、bool turn 与缺失语义字段，消除隐式截断与原生 TypeError；`record_settlement()` 严格校验 category 一致性，对等价重复结算幂等返回，对冲突结算显式报错；`load_effects()` 严格校验 entry 与 payload 的 `session_id`、`lane` 及 `operation_id` 边界一致性，并按严格时间序解析，拒绝重复 intent、拒绝无 intent 的孤立 settlement、拒绝冲突结算。
  - 操作启动与恢复失败完整生命周期清理与阻断：`_start_run_operation()` 严格校验 `RuntimeOperationStore.start()` 返回值与 `_settle_pending_effects_for_resume()`，在底层存储异常或恢复合成失败时立即 fail-closed 终止，派发标准 `task_done` 错误事件并清理 `RunRegistry` 状态与 work_item（移除多余的重复释放），阻断后续所有外部调用；`complete_or_block_work_item()` 纯净消费 `GhostWorkItem | None`，移除无用的包装对象兼容逻辑。
  ...
  - 工具时序与安全写入：调整时序为 `execute_tool_call -> record_tool_outcome -> record_tool_call_settlement`；`record_settlement_safely` 保证日志写失败绝不掩盖真实业务结果或异常；删除废弃的 `begin_tool_call()` 与无用的 `ReplayDecision.to_payload()`。
