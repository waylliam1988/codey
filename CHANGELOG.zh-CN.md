...
- 外部副作用 Intent / Settlement 闭环与 Replay Policy v1：
  ...
  - 严格 Schema 与 Fail-Closed：`from_payload()` 严格校验字段类型与长度，要求 `lane` 与 `operation_id` 为必填字段，拒绝未知 record_kind、bool turn 与缺失语义字段，消除隐式截断；`record_settlement()` 严格校验 category 一致性，对等价重复结算幂等返回，对冲突结算显式报错；`load_effects()` 严格校验 entry 与 payload 的 lane 及 operation_id 边界，并按严格时间序解析，拒绝重复 intent、拒绝无 intent 的孤立 settlement、拒绝冲突结算。
