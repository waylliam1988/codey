...
- 工具提示面收敛与运行时 Prompt 面薄追踪 v1：
  ...
  - 测试与回归：新增 `tests/test_tool_prompt.py`、`tests/test_tool_contract_drift.py`（锁定 `knowledge_link` exact title 真实能力并校验解析器修复分类）、`tests/test_prompt_surface.py`（覆盖 per-send `surface_id`、真实派生校验、拒绝长/非规范 phase 与 send_ref、强校验必填 epoch、bool-int 拒绝、无 `send_ref` 时 surfaces 为空与落盘字段恒等重算断言）与 `tests/test_golden_parity.py`（锁定 `SYSTEM_PROMPT`/控制块基线，并将 `test_research_repair_prompt_golden` 改为真实重算 `render_research_repair_prompt` + `append_block` 全量逐字节比对，改用 research 领域工具契约常量隔离跨域耦合）。全量 pytest 通过：`3467 passed, 4 skipped, 1235 subtests passed in 318.59s (0:05:18)`。Coding 模型可见字节完全保持不变，Research 动态示例诚实收敛真实能力且无死代码兼容包袱。
