...
def build_unified_research_graph(
    ...
    *,
    ...
    """Compose the concept and evidence read models for one session.

    Presentation only: nothing is persisted and neither layer's semantics
    are merged. Declared concept relations stay concept edges, evidence
    links stay note/source edges, and tag edges only connect visible notes
    to concepts. (Formerly ``UnifiedResearchGraphBuilder``; a class added
    nothing over these arguments, so it is a function.)
    """
    if store is None:
    ...
    evidence = KnowledgeGraphBuilder(store).build_for_session(
        session_id,
        focus_ids=focus_ids,
    ...
    concept = ConceptGraphBuilder(store).build_for_session(
        session_id,
        node_limit=node_limit,
        edge_limit=edge_limit,
    )
    ...
    _append_visible_note_tags(store, nodes_by_id, edges_by_id, evidence_note_ids)
...
def _append_visible_note_tags(
    ...
    for row in store.index.tags_for(note_ids, active_only=True):
