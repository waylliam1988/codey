...
:mod:`codey.repairs.adapter_surface`): one provider's driver plus the shared web
...
- ``profile_data``: ``profiles.json`` (data, so no snippet scan).
...
def classify_impact(changed_files: list[str] | set[str] | tuple[str, ...]) -> tuple[str, ...]:
    ...
    for rel in changed_files:
        ...
        if normalized == "codey/providers/profiles.json":
