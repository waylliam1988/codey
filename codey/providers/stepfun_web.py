...
from codey import cancellation, stepfun
from codey.browser import DEFAULT_PORT, DEFAULT_PROFILE, Session, open_stepfun
...
@dataclass
class StepFunWebProvider:
    session: Session

    name: ClassVar[str] = "StepFun Chat"
    ...
    def new_chat(self, timeout: float | None = None) -> None:
        ...
        with cancellation.deadline_scope(start_deadline(timeout)):
            run_provider_action(
                ...
                func=lambda: stepfun.new_chat(self.session.page, **kwargs),
            )
    ...
    def send(self, text: str, timeout: float | None = None) -> str:
        ...
        with cancellation.deadline_scope(start_deadline(timeout)):
            return run_provider_action(
                ...
                action="send",
                page=self.session.page,
                func=lambda: stepfun.chat(self.session.page, text, **kwargs),
            )
