...
class ProviderSelectorUiTests(unittest.TestCase):
    ...
    def test_research_context_is_explicit_and_user_facing(self) -> None:
        ...
        self.assertIn("function normalizeStoredResearchRun(run)", HTML)
        ...
        self.assertIn("function loadResearchGraph(run, sessionId, canvas, status, detail)", HTML)
        self.assertIn("function drawResearchGraph(canvas, graph, detail, run, sessionId)", HTML)
        self.assertIn("fetch('/api/research/graph?' + params.toString())", HTML)
        self.assertIn("let researchGraphLoadSeq = 0;", HTML)
        self.assertIn("researchGraphLoadSeq += 1;", HTML)
        self.assertIn("const seq = ++researchGraphLoadSeq;", HTML)
        self.assertIn("seq !== researchGraphLoadSeq || !canvas.isConnected", HTML)
        self.assertIn("const runtime = drawResearchGraph(canvas, graph, detail, run, sessionId);", HTML)
        self.assertIn("if (runtime && typeof runtime.stop === 'function') runtime.stop();", HTML)
        self.assertIn(".research-graph-status.error { color: var(--err-text); }", HTML)
        self.assertIn("function setResearchGraphStatus(status, text, isError = false)", HTML)
        self.assertIn("setResearchGraphStatus(status, 'Loading graph...');", HTML)
        self.assertIn("setResearchGraphStatus(status, 'No graph yet');", HTML)
        self.assertIn("setResearchGraphStatus(status, data.error || 'Graph unavailable', true);", HTML)
        self.assertIn("setResearchGraphStatus(status, 'Graph unavailable', true);", HTML)
        self.assertIn("status.classList.toggle('error', !!isError);", HTML)
        self.assertIn("status.classList.remove('error');", HTML)
        self.assertIn("status.hidden = false;", HTML)
        self.assertIn("status.textContent = text;", HTML)
        self.assertIn("research-graph-stage", HTML)
        self.assertIn("research-depth", HTML)
        self.assertIn("Reset", HTML)
        self.assertIn("ctx.setLineDash([5, 5])", HTML)
        self.assertIn("ctx.strokeStyle = hoverActive ? colors.ok", HTML)
        self.assertIn("ctx.fillStyle = hovered ? colors.ok", HTML)
        self.assertIn("action.onclick = () => openResearchGraphNode(node, run, sessionId);", HTML)
        self.assertIn("function onDblClick(event)", HTML)
        self.assertIn("if (hit) openResearchGraphNode(hit, run, sessionId);", HTML)
        self.assertIn("setResearchGraphDetail(detail, hit, run, sessionId);", HTML)
        self.assertNotIn("if (hit && hit.url) openResearchGraphNode(hit, run, sessionId);", HTML)
        self.assertNotIn("node.focus ? colors.ok", HTML)
        self.assertNotIn("graph: data.graph", HTML)
