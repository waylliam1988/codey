...
The first DeepSeek attempt failed before task entry because CDP port 9222 was
...
`task_entry` claim path, while mode bodies are safe stubs so it does not edit
...
The harness uses production `task_entry` paths with safe Research stubs. It
...
provider per fresh webpage tab. The harness validates the real `task_run`
...
It uses the production `task_entry` chat path and real provider replies, but
