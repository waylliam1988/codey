...
decision. The production-spine probe runs the real `TaskFlow` routing entry
...
uses the production `TaskFlow` entry point and real queue claim/complete/block
...
The first DeepSeek attempt failed before TaskFlow because CDP port 9222 was
...
`TaskFlow` claim path, while mode bodies are safe stubs so it does not edit
...
The harness uses production `TaskFlow` paths with safe Research stubs. It
...
provider per fresh webpage tab. The harness validates the real `TaskFlow`
...
It uses the production `TaskFlow` chat path and real provider replies, but
