"""Live / End-to-End A/B harness for Tool Argument Repair (0.5.2 Baseline vs 0.5.3 Canonicalization).

Evaluates repair turn savings, prompt token economy, and parser safety boundaries
across representative multi-turn task scenarios and model dialect variations.
"""
...
class LiveScenario:
    id: str
    description: str
    model_turns: list[str]  # Simulated or recorded live model turns
...
def run_live_ab() -> dict[str, Any]:
    ...
    total_scenarios = len(LIVE_SCENARIOS)
    variant_a_turns = 0
    variant_b_turns = 0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Live A/B tool argument repair comparison.")
    ...
    print("=" * 70)
    print("0.5.3 Tool Argument Canonicalization Live A/B Evaluation")
    print("=" * 70)
