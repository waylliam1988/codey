...
class TaskAuthorizationTests(unittest.TestCase):
    ...
    def test_untouched_test_wording_stays_unauthorized(self) -> None:
        ...
        for task in (
            ...
            "Do not modify the tests. Fix the implementation only.",
            "Don't change tests.",
            "Change implementation, not tests.",
            "Modify code, not tests.",
            "Only change implementation; tests should stay unchanged.",
            "Update src/mod.py without changing tests.",
            "Never touch the tests; fix src/mod.py.",
            "把测试改成新的返回值",
            "改一下测试",
            "不要修改测试，只修实现",
            "只改实现，不改测试",
